...
function actionTrigger() {
  $.ajax({
    url: `//logicrasacbsvr.southindia.cloudapp.azure.com:5005/conversations/${sender_id}/execute`,
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
      name: action_name,
    ...
    success(botResponse, status) {
      console.log("from actionTrigger()");
      console.log("Response from Rasa: ", botResponse, "\nStatus: ", status);
    ...
    error(xhr, textStatus) {
      ...
      setBotResponse("");
      console.log("Error from  trigger depricated API");
      console.log("Error from bot end: ", textStatus);
      $("#userInput").prop("disabled", false);
    },
  });
...
function customActionTrigger() {
  const httpUrl = '//logicrasacbsvr.southindia.cloudapp.azure.com:5005/webhooks/rest/webhook';
  if (window.location.protocol === 'http:') {
   	console.log('The current page is using HTTP.');
       $.ajax({
        url: httpUrl,
        type: "POST",
        contentType: "application/json",
        ...
        success(botResponse, status) {
          ...
          const requestUrl = '//logicrasacbsvr.southindia.cloudapp.azure.com:5005/webhooks/rest/webhook';
          console.log("Success! URL: ", requestUrl, "\nResponse from Rasa: ", botResponse, "\nStatus: ", status);
          console.log("Response from Rasa: ", botResponse, "\nStatus: ", status);
        ...
        error(textStatus) {
          console.log("from webhook()");
          const requestUrl = '//logicrasacbsvr.southindia.cloudapp.azure.com:5005/webhooks/rest/webhook';
          console.log("Error! URL: ", requestUrl, "\nError from bot end: ", textStatus);
          setBotResponse("");
          console.log("Error from webhooks/rest");
          console.error("Error from bot end: ", textStatus);
           $("#userInput").prop("disabled", false);
        },
      });
  } else if (window.location.protocol === 'https:') {
    console.log('The current page is using HTTPS.');
    const modifiedUrl = httpUrl.replace(/^(\w+:)?\/\//, 'http://');
      $.ajax({
        url: modifiedUrl,
        type: "POST",
        contentType: "application/json",
...
function actionIntentTigger() {
    const httpUrl = '//logicrasacbsvr.southindia.cloudapp.azure.com:5005/conversations/default/trigger_intent';
  if (window.location.protocol === 'http:') {
   	console.log('The current page is using HTTP.');    
    $.ajax({
        url: httpUrl,
        ...
        success(botResponse, status) {
        console.log("from welcome Intent");
        const requestUrl = '//logicrasacbsvr.southindia.cloudapp.azure.com:5005/conversations/default/trigger_intent';
        console.log("Success! URL: ", requestUrl, "\nResponse from Rasa: ", botResponse, "\nStatus: ", status);
        ...
        console.log("Error! URL: ", requestUrl, "\nError from bot end: ", textStatus);
        setBotResponse("");
        console.log("Error from welcome msg");
        console.log("Error from bot end: ", textStatus);
        $("#userInput").prop("disabled", false);
        },
    });
