        ...
        if response.status_code == 200:
            ...
            for item_data in items:
                if item_data.get('item') == self.item_loc:
                    ...
                    output.append(item_data_dict)
                    price_string = ", <br>".join([f"{key}: {value}" for key, value in item_data_dict.items()])
                    price_u1_details += f"\n{price_string}\n"
                    price_u1_data.append(item_data_dict)
...
# ------------
            for order in api_response:
                ...
                for item in order['poItemTbl']:
                    ...
                    order_string = ", <br>".join([f"{key}: {value}" for key, value in order_details.items()])
...
# 
                inventory_string = ", <br>".join([f"{key}: {value}" for key, value in item_info.items()])
...
# # local ends
    ...
    def item_details(self, item_no, access_token):
        ...
        if response.status_code == 200:
            ...
            item_details += f"Item Grandparent : {itemGrandparent} <br>Item Parent : {itemParent}<br> Item : {item}<br> Item Description : {itemDesc}<br> Status : {status}<br> Item Level : {itemLevel}<br> Tran Level : {tranLevel}"
