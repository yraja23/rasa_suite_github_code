...
##local code ends
    ...
    def supplier_details(self, supplier_no, access_token):
        ...
        if response.status_code == 200:
            ...
            supplier_details += f"Supplier: {supplier}<br>Supplier Name: {supName}<br>Currency Code: {currencyCode}<br>VAT Region: {vatRegion}<br>Freight_terms: {freight_terms}"
...
# #local starts
                ...
                return supplier_data, file_url, supplier_details
# # local ends
...
class allFunc:
    ...
    g_lang = None # Declare the global class variable to get the full detected
    # def __init__(self):
...
#-------------------------------------------------------
    ...
    def langToEng(self, keyword, language):
        ...
        allFunc.g_lang = language
        return text_val, user_lang, language
    ...
    def Eng_to_user_language(self, response, lang, lang_name):
        ...
        if lang is None:
            lang = 'en'
        # calling the text translation part from geminipro file
        translateObj = Geminipro()
        output = translateObj.Eng_to_user_language1(response, lang_name)
